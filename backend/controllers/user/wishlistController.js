...
const addToWishlist = async(req, res)=>{
    const {id} = req.body
    const [wishlistItem] = await db.execute('SELECT * FROM wishlist WHERE user_id=? AND product_id=?',[Number(req.user.id), id])
    ...
    if(wishlistItem.length > 0) {
        ...
        await db.execute('INSERT INTO wishlist (id, user_id, product_id) VALUES (?,?,?)',[uniqueID, Number(req.user.id), id])
