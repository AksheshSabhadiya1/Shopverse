...
checkoutRouter.post('/checkout', async(req, res)=>{
    if(req.user){
        const {firstname, lastname, floor, address, city, pincode, country, email, mobile, isSaveInfo, paymentType, subtotal, shippingCharge, total, productIDs} = req.body
        const uniqueID = Math.floor(Math.random() * 1e16)
        const uniqueID2 = Math.floor(Math.random() * 1e16) 

        await db.execute("INSERT INTO checkout (id, user_id, firstname, lastname, email, floor, address, city, pincode, country, mobile, isSaveInfo, paymentType, subtotal, shippingCharge, total, allProducts_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",[uniqueID, req.user.id,firstname,lastname,email,floor,address,city,pincode,country,mobile,isSaveInfo,paymentType,subtotal,shippingCharge,total, JSON.stringify(productIDs)])
