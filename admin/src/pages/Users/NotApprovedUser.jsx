...
export default function NotApprovedUser() {
    ...
    const fetchNotApprovedUser = async () => {
        try {
            const { data } = await axios.get('http://localhost:5000/admin/users', {
                withCredentials: true
            })
            const result = data.filter(user => user.approved_status === 0)
            result.length > 0 ? setUsers(result) : []
        } catch (error) {
            console.log("Error while fetching notapproved user data");
        }
    }

    useEffect(() => {
        fetchNotApprovedUser()
    }, [])
                ...
                <div className="w-full max-w-6xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {users.length > 0 ? (
                        ...
                        <p className="text-center text-gray-300 w-full col-span-4">
                            NotApproved Users not available.
                        </p>
                    )}
                </div>
            </div>
