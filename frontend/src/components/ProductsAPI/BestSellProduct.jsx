                ...
                {data?.map((product) => (
                    <div key={product.id} className="bg-white rounded p-4 min-w-[250px] transition duration-300 ease-in-out" onMouseEnter={() => setCartBtnVisible(product.id)} onMouseLeave={() => setCartBtnVisible(null)}>
                        ...
                        <div className="h-50 w-full flex flex-col items-center">
                            ...
                            />
                            {path === 'wishlist' && <button className="w-full flex justify-center transition-all duration-300 ease-in-out items-center mt-2 bg-black relative -top-10 text-white py-2 cursor-pointer rounded-b hover:bg-red-600">
                            <ShoppingCart className="me-2" />Add to Cart</button>}
